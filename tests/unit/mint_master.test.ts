...
const expectMintMasterState = async (
  mintMasterAccount: PublicKey,
  portal: PublicKey,
...
const getMintAuthority = async (mint: PublicKey) => {
  return (await getMint(provider.connection, mint, null, TOKEN_2022_PROGRAM_ID))
    .mintAuthority;
};
...
describe("MintMaster unit tests", () => {
  ...
  describe("mint_m unit tests", () => {
    ...
    beforeEach(async () => {
      ...
      mintMasterAccount = await initialize(
        portal.publicKey,
        distributor.publicKey
      );
