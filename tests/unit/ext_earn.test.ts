...
const getTokenBalance = async (tokenAccount: PublicKey) => {
    ...
    const tokenAccountInfo = await getAccount(provider.connection, tokenAccount, null, TOKEN_2022_PROGRAM_ID);
    if (!tokenAccountInfo) {
...
const prepClaimFor = async (signer: Keypair, earner: PublicKey, earnManager: PublicKey, earnerTA?: PublicKey, earnManagerTA?: PublicKey) => {
...
const claimFor = async (earner: PublicKey, earnManager: PublicKey, balance?: BN) => {
    ...
    // Setup the instruction
    await prepClaimFor(earnAuthority, earner, earnManager, earnerState.recipientTokenAccount ?? earnerATA, earnManagerState.feeTokenAccount);
