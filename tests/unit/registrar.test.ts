...
const removeFromList = async (list: string, address: PublicKey) => {
  // Setup the instruction call
  const { list: formattedList, flag } = prepRemoveFromList(
    admin,
    list,
    address
  );
...
describe("Registrar unit tests", () => {
  ...
  describe("set_key unit tests", () => {
    ...
    test("Admin can set a new key", async () => {
      // Setup the instruction call
      const { key, value, register } = prepSetKey(
        admin,
        "test-key",
        "test-value"
      );
    ...
    test("Admin can update an existing key", async () => {
      ...
      const {
        ...
        value: newValue,
        register,
      } = prepSetKey(admin, "test-key", "new-test-value");
    ...
    test("Non-admin cannot update an existing key", async () => {
      ...
      const {
        ...
        value: newValue,
        register,
      } = prepSetKey(nonAdmin, "test-key", "new-test-value");
      ...
      await expectRegisterState(
        register,
        toFixedSizedArray(Buffer.from("test-value"), 32)
      );
    });
