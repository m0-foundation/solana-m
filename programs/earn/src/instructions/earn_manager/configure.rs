...
// local dependencies
use crate::{
    constants::{ANCHOR_DISCRIMINATOR_SIZE, ZERO_BIT, ONE_HUNDRED_PERCENT, MINT},
    errors::EarnError,
    state::{
        Global, GLOBAL_SEED,
...
pub fn handler(
    ...
    let leaf = solana_program::keccak::hashv(&[&[ZERO_BIT],&ctx.accounts.signer.key().to_bytes()]).to_bytes();
    if !verify_in_tree(
        proof,
