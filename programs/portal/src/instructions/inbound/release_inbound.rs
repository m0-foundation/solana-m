...
use anchor_spl::{
    token_2022::spl_token_2022::{instruction::mint_to, onchain::invoke_transfer_checked},
    token_interface,
};
use ntt_messages::mode::Mode;

use crate::{errors::PortalError, state::*, utils::TOKEN_AUTHORITY_SEED};
...
pub struct ReleaseInbound<'info> {
    ...
    /// CHECK: the token program checks
    #[account(
        mut,
...
pub fn release_inbound_mint_multisig<'info>(
    ...
    let token_authority_sig: &[&[&[u8]]] =
        &[&[TOKEN_AUTHORITY_SEED, &[ctx.bumps.common.token_authority]]];
...
fn release_inbox_item(
    ...
    if inbox_item.try_release()? {
        ...
        Err(PortalError::CantReleaseYet.into())
    } else {
        Ok(None)
    }
}
