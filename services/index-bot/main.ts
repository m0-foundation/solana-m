...
async function sendIndexUpdate(options: ParsedOptions) {
    ...
    const [, quote] = await portal.read.quoteDeliveryPrice([1, ('0x' + Buffer.from([1]).toString('hex')) as `0x${string}`]);
    if (options.dryRun) {
        logger.info('Bridge price quote: ', quote.toString());
    }

    // Send index update transaction
    const refundAddress = ('0x' + options.evmClient.account?.address.slice(2).padStart(64,'0')) as `0x${string}`;
    if (options.dryRun) {
        logger.info('Destination chain (wormhole ID): ', 1);
        logger.info('Refund address: ', refundAddress);
        logger.info('Ethereum account: ', options.evmClient.account?.address);
        logger.info('Ethereum chain: ', options.evmClient.chain);
    } else {
